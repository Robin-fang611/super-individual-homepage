...
- `shots/`：浏览器截图（realm-road）。
- `last-smoke.json`：最近一次冒烟的结构化结果。

## 验证方式（真实浏览器，非仅截图）
两个自动化都用 `scripts/smoke-road.mjs` 驱动已缓存的无头 Chrome
（`puppeteer-core`，仅装在管理 node workspace，不影响站点零依赖规则），
确认星图正常启动、临近交互提示元素（`#interact-prompt`）存在、无控制台 / 页面报错。
- 设计迭代自动化（A）：每轮改动后跑 `node --test` + `validate-content` + 冒烟，全绿才提交。
- 可视化校验自动化（B）：每 30 分钟（错峰 15 分钟）复跑冒烟，产出 `health-NN.md`。
